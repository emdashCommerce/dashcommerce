...
import { defineProductsCollection, defineProductTaxonomies } from "@dashcommerce/core";

interface DemoProductEntry {
	id: string;
	slug: string;
	status: "published";
...
/**
 * Default terms for the `product_category` taxonomy. These match the
 * category slugs linked from the storefront's home-page bento grid so the
 * existing `/category/{slug}` URLs keep working out of the box. Editors
 * can add / rename / reorganize categories from the admin UI afterwards.
 */
const productCategoryTerms: Array<{
	slug: string;
	label: string;
	description?: string;
	parent?: string;
...
const demoProducts: DemoProductEntry[] = [
	{
		id: "enamel-mug",
		slug: "enamel-mug",
		status: "published",
...
// Representative product_tag terms. Tags cut across categories — they're
// the merchandising labels shoppers filter by ("new arrival", "sale",
// "bestseller"). Merchants can add / rename / remove these from the admin.
const productTagTerms: Array<{
	slug: string;
	label: string;
	description?: string;
}> = [
